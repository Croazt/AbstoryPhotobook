          ...
          <div class="invoice">
            ...
            <div class="text-md-right">
              <div class="float-lg-left mb-lg-0 mb-3">
                ...
                <a href="http://127.0.0.1:8000/order/{{$order->id}}/2/update" class="btn btn-outline-info btn-icon icon-left" onclick="return confirm('Konfirmasi untuk update status pesanan?')">Menunggu Link</a>
                <?php } ?>
                <?php if($order->status!="Sedang Diproses"){?>
                <a href="http://127.0.0.1:8000/order/{{$order->id}}/3/update" class="btn btn-outline-warning btn-icon icon-left" onclick="return confirm('Konfirmasi untuk update status pesanan?')">Sedang Diproses</a>
                <?php } ?>
                <?php if($order->status!="Pesanan Dikirim"){?>
                <a href="http://127.0.0.1:8000/order/{{$order->id}}/4/update" class="btn btn-outline-dark btn-icon icon-left" onclick="return confirm('Konfirmasi untuk update status pesanan?')">Pesanan Dikirim</a>
                <?php } ?>
                <?php if($order->status!="Pesanan Selesai"){?>
                <a href="http://127.0.0.1:8000/order/{{$order->id}}/5/update" class="btn btn-outline-success btn-icon icon-left" onclick="return confirm('Konfirmasi untuk update status pesanan?')">Pesanan Selesai</a>
                <?php } ?>                                
              </form>          
              </div>
              <br>
